...
inst_needs_relocation:
	# YOUR CODE HERE
	# NOTE: Only j and jal use concrete addess and thus require relocation
	srl $t0, $a0, 26
	addi $t1, $0, 2
	addi $t2, $0, 3
	beq $t0, $t1, inst_needs_relocation_true
	beq $t0, $t2, inst_needs_relocation_true
	add $v0, $0, $0
	j inst_needs_relocation_finale
inst_needs_relocation_true:
	addi $v0, $0, 1
inst_needs_relocation_finale:
	jr $ra
...
#  $a1 = the byte offset of the instruction in the current file (addr of the instruction to be recloated)
#  $a2 = the symbol table
...
relocate_inst:
	...
	# check whether the addr is in the relocation table
	move $a0, $a3
	jal symbol_for_addr
	beq $v0, $0, relocate_inst_error
	# check whether the symbol is in the symbol table
	lw $a0, 12($sp)
	move $a1, $v0
	jal addr_for_symbol
	li $t0, -1
	...
	move $t0, $v0
	srl $t0, $t0, 2	# address in the symbol table has bytes form
	lw $t1, 4($sp)
