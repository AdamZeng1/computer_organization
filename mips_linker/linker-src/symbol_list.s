...
addr_symbol_loop:
	...
	lw $a0, 4($s0) # name in the list node
	jal streq
	beq $v0, $0, addr_found
	move $a1, $s1	# streq modifies the value of $a1!!
	lw $s0, 8($s0)
	j addr_symbol_loop
addr_found:
	lw $v0, 0($s0)
...
add_to_list:	
	...
	sw $a0, 4($sp)
	sw $s0, 8($sp)
	sw $a1, 12($sp)	# strncpy modifies the value of $a1 !!
	...
	lw $t0, 4($sp)
	sw $v0, 4($s0)	# name of symbol
	lw $t1, 12($sp)
	sw $t1, 0($s0)	# address of symbol
	sw $t0, 8($s0)	# pointer to the next node

	move $v0, $s0	# return the new list 
	lw $ra, 0($sp)
	lw $a0, 4($sp)
	lw $s0, 8($sp) 
	lw $a1, 12($sp)
