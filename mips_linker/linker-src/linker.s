...
write_machine_code_next_inst:
	...
	# YOUR_INSTRUCTIONS_HERE
	move $a0, $s6	# instruction that needs relocating
	move $a1, $s5	# byte offset of the instruction in the current file
	move $a2, $s2	# symbol table
	move $a3, $s3	# relocation table
	jal relocate_inst
	move $s6, $v0
